...
from alpha_trigger_locations import pvr_locations
import re
import mne
import csv
import numpy
...
class Participant:
    def __init__(self, pid: str, filename: str):
        ...
        if 'S2' in filename:
            self.event_id = dict((k, trigger_codes[k]) for k in ('S2-EO', 'S2-EC'))
        else:
            self.event_id = dict((k, trigger_codes[k]) for k in ('S5-EO', 'S5-EC'))
        self.events = numpy.empty(1)

    def preprocess(self):
        raw_file = 'Post VR/' + self.filename + '.vhdr'
        outfile = 'Post VR/processed/' + self.filename + '.raw.fif'
    ...
    def generate_events(self):
        raw_file = 'Post VR/processed/' + self.filename + '.raw.fif'
        raw: mne.io.Raw = mne.io.read_raw_fif(raw_file,
                                              preload=True)

        if 'S2' in self.filename:
            keys = pvr_locations[self.pid][:2]
        else:
            keys = pvr_locations[self.pid][2:]
    ...
    def extract_epochs(self):
        raw = mne.io.read_raw_fif('Post VR/processed/{0}.raw.fif'.format(self.filename), preload=True)
        tmin = 0
        tmax = 2
        ...
        try:
            ...
            rejections.savefig('Post VR/figures/{0}_dropped.png'.format(self.filename))
        except Exception:
            print(self.filename, 'has no drops!')

        try:
            epochs.save('Post VR/epochs/{0}-epo.fif'.format(self.filename), overwrite=True)
        except IndexError:
    ...
    def time_frequency(self):
        ...
        epochs = mne.read_epochs('Post VR/epochs/' + self.filename + '-epo.fif')
        ...
        for b in bands:
            ...
            try:
                ...
                tf_means.to_csv('Post VR/time_freq/{0}_{1}.csv'.format(self.pid, b), index=False)
            except ValueError:
...
csvs = glob('Post VR/time_freq/*alpha.csv')
dfs = []
for file in csvs:
    df = pandas.read_csv(file, index_col=False)
    dfs.append(df)
joined_frame = pandas.concat(dfs, ignore_index=True)
joined_frame.to_csv('Post VR/time_freq/alpha_combined.csv')

csvs = glob('Post VR/time_freq/*theta.csv')
dfs = []
for file in csvs:
    df = pandas.read_csv(file, index_col=False)
    dfs.append(df)
joined_frame = pandas.concat(dfs, ignore_index=True)
joined_frame.to_csv('Post VR/time_freq/theta_combined.csv')

csvs = glob('Post VR/time_freq/*_combined.csv')
dfs = []
for file in csvs:
    ...
    dfs.append(df)
joined_frame = pandas.concat(dfs, ignore_index=True)
joined_frame.to_csv('Post VR/time_freq/alpha_theta_combined.csv')
