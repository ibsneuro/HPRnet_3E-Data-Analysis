...
dat <- read.csv('VR Task/time_freq/alpha_theta_combined.csv')
session.2 <- c('S2-TO', 'S2-B0', 'S2-B1', 'S2-B2', 'S2-B3', 'S2-B4', 'S2-B5')
session.5 <- c('S5-TO', 'S5-B0', 'S5-B1', 'S5-B2', 'S5-B3', 'S5-B4', 'S5-B5', 'S5-B6')
...
#subset(ae.m.df, cond %in% session.2)
o.plot <- ggplot(subset(ae.m.df, cond %in% c('S2-B4', 'S5-B4')), aes(x=epoch, y=fit, group=cond)) +
...
o.plot <- ggplot(subset(ae.m.df, cond %in% session.5), aes(x=epoch, y=fit, group=cond)) +
  geom_ribbon(aes(ymin=fit-se, ymax=fit+se), alpha = 0.1, fill = "grey70") +
  geom_line(aes(colour=cond), alpha = 1) +
  theme_light() +
  theme(text=element_text(size=10,  family="serif")) +
